...
//代表节点
type node struct {
	isLast   bool                //该节点是否能称为一个独立的uri，是否自身就是一个终极节点
...
func (n *node) matchNode(uri string) *node {
	...
	segment := segments[0]
	if !isWildSegment(segment) {
		segment = strings.ToUpper(segment) //大小写
	}
	//匹配符合的下一层子节点
	...
	if len(segments) == 1 {
		//如果segement已经是最后一个节点，判断这些cnode是否有isLast标记
